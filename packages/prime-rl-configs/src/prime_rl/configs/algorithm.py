...
class BaseAlgorithmConfig(BaseConfig):
    ...
    def validate_references(self):
        if isinstance(self, SFTAlgorithmConfig) and self.sampling.source == "policy":
            raise ValueError(
                "algorithm 'sft' needs a non-policy source for its supervised tokens — CE on the "
...
class SFTAlgorithmConfig(BaseAlgorithmConfig):
    ...
    """The frozen-teacher shorthand folds into this algorithm's sampling source."""
