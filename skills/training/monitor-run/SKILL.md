...
| trainer | `sparse_update/{sparsity,changed_numel,patch_bytes,patched_tensors}` | sparse filesystem weight-sync metrics when `trainer.weight_broadcast.type = "sparse_filesystem"` (`kernel_format = true` for GPU-direct application without CPU cache) |
